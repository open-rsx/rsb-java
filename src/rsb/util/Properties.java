...
public class Properties {
    ...
    private final static String[] boolValues = {"FALSE", "TRUE", "YES", "NO", "true", "false", "yes", "no", "1", "0"};
	...
	propsMap.put("qualityofservice.reliability", new InSetProperty("RELIABLE", reliabilityValues));
	String[] orderedValues = { "ORDERED", "UNORDERED" };
	propsMap.put("qualityofservice.ordering", new InSetProperty("ORDERED", orderedValues));

	String[] errorValues = { "LOG", "PRINT", "EXIT" };
	propsMap.put("errorhandling.onhandlererror", new InSetProperty("LOG", errorValues));
	...
	propsMap.put("transport.spread.converter.java.string", new ValidProperty("String"));
    }

    public static Properties getInstance() {
	if(singleton == null) {
	    // Read configuration properties in the following order:
	    //  1. from ${HOME}/.config/rsb.conf, if the file exists
	    //  2. from $(pwd)/rsb.conf, if the file exists
	    //  3. from environment
		...
		System.out.println("Found rsb.conf in configuration directory '" + homeDir + "/.config'");
	    } catch(FileNotFoundException ex) {
		System.out.println("No rsb.conf found in configuration directory '" + homeDir + "/.config'");
		...
		System.err.println("Caught IOException trying to read " + workDir + "rsb.conf: " + ex.getMessage());
	    }
	    singleton.loadEnv();
	...
	while (reader.ready()) {
	    ...
	    index = line.indexOf('=');
	    if (index == -1) {
		System.err.println("Illegal line in configuration file: `" + line + "'");
		continue;
	    }
	    String key = section + "." + line.substring(0, index).trim();
	    if ((key.startsWith("transport.spread.converter")
		 && !key.startsWith("transport.spread.converter.java"))
		|| (key.startsWith("transport")
		    && !key.startsWith("transport.spread"))
		|| (key.startsWith("plugins")
		    && !key.startsWith("plugins.java"))) {
		continue;
	    }
