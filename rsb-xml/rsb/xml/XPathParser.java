...
public class XPathParser {
	...
	// this is done for further development, we "just" need to verify that the filter is valid 
	private XPath toXPathExpression(String filter) throws XPathNotSupportedException {
		...
		if (xpath.charAt(0)=='/' && xpath.length()==1) {
			...
			if (xpath.charAt(0)=='/' && hasFilterOnRoot(path)==false) {
				...
				case 1: 
					if (rootEnd == xpath.length()-1){
						throw new XPathNotSupportedException("XPath "+path+" currently not supported");
				...
				if (xpath.charAt(0)=='/' && hasFilterOnRoot(path)==true) {	
